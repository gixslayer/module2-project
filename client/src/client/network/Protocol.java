...
public class Protocol {
    ...
    public Protocol(TcpClient c, Client client){
        this.client = c; this.clientm = client;
    }
    //Send
    ...
    public void handleDoneMove(String[] args){
        if(args.length < 2) {
            client.send(ERR_SYNTAX);
        }else if(args[1].matches("[0-9]")&& (args[0].equals(clientm.getClientName()) || args[0].equals(clientm.getOpponent()))) {
