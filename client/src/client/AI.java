...
public class AI {
    ...
    public AI (ClientController c){
        ...
        this.lookahead = 6;
        //TODO mak static
        current_state = new State(true);
    }
    public void doMove( Boolean myTurn){
        current_state = new State( myTurn);

        createDecisionTree(current_state);
        calculateChanses(current_state);
    ...
    public ArrayList<State> getBestMove(){
        ...
        for (State t : current_state.getNextStep()){
            if (temp.size()<1){
                temp.add(t);
    ...
    public void createDecisionTree (State state){
        current_state.syncTo(clientController.getBoard());
        Disc disc;
        Vector result = new Vector();
        for (int i=0; i<state.getColumns(); i++) {
            State temp = new State(!state.isMyTurn());
            temp.syncTo(state);
            if (temp.isMyTurn()){ disc = Disc.Red;
            }else{
                disc = Disc.Yellow;
            }
...
//TODO remove test main
    public static void main (String[] args){
        ...
        ai.calculateChanses(ai.current_state);
        for (State t : ai.current_state.getNextStep()){
            System.out.println(t.getV());
        }
    }
